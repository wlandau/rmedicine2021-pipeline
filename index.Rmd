...
This technique is similar to simulation-based calibration (SBC) [@cook2006; @talts2020]. SBC may be more robust than the interval-based validation technique above, but the latter is currently easier to perform with `stantargets` and easier to communicate for high-dimensional models.^[Parameter-specific rank statistic histograms can be difficult to inspect when there are many parameters.]
...
tar_stan_mcmc_rep_summary(
  ...
  batches = 200, # Number of branch targets.
  reps = 10, # Number of model reps per branch target.
  chains = 4, # Number of MCMC chains.
  parallel_chains = 4, # How many MCMC chains to run in parallel.
  iter_warmup = 2e4, # Number of MCMC warmup iterations to run.
  iter_sampling = 2e4, # Number of MCMC post-warmup iterations to run.
  summaries = list(
...
After we fit the model, we assess convergence using univariate Gelman-Rubin potential scale reduction factors [@bda3] on all scalar parameters across all simulations. Below, we report the maximum Gelman factor of each visibly divergent simulation rep (i.e. with a Gelman factor above 1.01).
...
Finally, we report coverage statistics. If the model is implemented correctly, `coverage_50` should not differ systematically from 0.5, and `coverage_95` should not differ systematically from 0.95. Coverage appears nominal in the table below. If there is doubt, simply increase the number of batches or reps per batch in `tar_stan_mcmc_rep_summary()` and run the pipeline again.
