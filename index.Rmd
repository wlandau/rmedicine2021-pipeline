...
simulate_data <- function(n_patients = 50, n_visits = 3, n_arms = 2) {
  ...
  while (is.null(sigma) || max(sigma) > 10) {
    ...
    lambda <- t(chol(rho))
    sigma <- rhcauchy(n = n_visits, location = 0, scale = 1)
...
Now, we define the targets for the steps of the workflow. We use the `stantargets` R package to manage most of the work: simulate 10000 datasets, analyze each dataset with the model, and calculate the coverage status of each simulation. We divide these 10000 replications among 100 batches for computational efficiency. As explained at <https://docs.ropensci.org/stantargets/articles/simulation.html>, all this can be done with the `tar_stan_mcmc_rep_summary()` function. The targets created by `tar_stan_mcmc_rep_summary()` can take a long time to run, so it is recommended to suppress interactive mode.
...
tar_stan_mcmc_rep_summary(
  ...
  reps = 100, # Number of model reps per branch target.
